...
learning_rate_initial = 1e-3    # Base learning rate for the ADAM optimizers; may be decreased over time
learning_rate_decay = 1000.     # How many steps to reduce the learning rate by a factor of e
learning_rate_minimum = 1e-3    # Floor for the learning rate
...
# Penalty params
PIN_penalty_mode = ['MSE', 'CE'][0]
gamma_target = 1.0 # Target ration of L(G(y))/L(x_trn)
lambda_pin_value = 10.   # Scaling factor of penalty for label mismatch
...
def pin_cnn(input, lbls = None, n_labels=None, reuse = False, encoded_dimension=16, cnn_layers=4,
    ...
    output, dec_vars = Decoder(z_pin, input_channel=image_channels, repeat_num=cnn_layers,
            hidden_num=node_growth_per_layer, data_format=data_format, reuse=reuse, final_size=scale_size)
...
x_trn = imgs
z_trn, x_out_trn, enc_vars, dec_vars = pin_cnn(input=x_trn, lbls = img_lbls if lambda_pin_value > 0. else None, n_labels=n_labels, reuse=False, 
...
z_gen, x_out_gen, _, _ = pin_cnn(input=x_gen, n_labels=n_labels, reuse=True, encoded_dimension=encoded_dimension, cnn_layers=cnn_layers, node_growth_per_layer=node_growth_per_layer, data_format=data_format, image_channels=image_channels)
...
for step in xrange(1, training_steps + 1):
    ...
    if print_cycle:
        ...
        if image_print_cycle:
            output = sess.run([x_trn[:8, :, :, :], x_gen[:8, :, :, :], x_ins, x_oos, x_demo, 
                               x_out_trn[:8, :, :, :], x_out_gen[:8, :, :, :], x_out_ins, x_out_oos, x_out_demo])
            ...
            plot_names = ['In-Sample Production', 'Generated', 'In-Sample (fixed)', 'Out-of-Sample (fixed)', 'Manipulated']
            ...
            # Print some individuals just to test label alignment
            i, l, zed = sess.run([imgs, img_lbls, z_trn])
            plt.figure(figsize=[8, 8])
            plt.subplot(1, 3, 1)
            plt.imshow(1. - i[:8, :, :, :].reshape([-1, image_size, 3]), interpolation='nearest')
            plt.subplot(1, 3, 2)
