...
class SGAN(object):
    def __init__(self, sess, max_iter=10001, optim='adagrad', learning_rate=1e-2,
            ...
            grid_gran=21, grid_n=None, dataset='gaussian', expt='test'):
    ...
    def build_model(self):
        ...
        self.d_scores_grid = tf.nn.tanh(
                common.ffnn(
                    tf.convert_to_tensor(self.grid),
                    **self.d_params))
        self.d_scores_real = tf.nn.tanh(
                common.ffnn(
                    self.data_sample, **self.d_params1))
        self.d_scores_gen = tf.nn.tanh(
                common.ffnn(
                    self.g, **self.d_params2))
